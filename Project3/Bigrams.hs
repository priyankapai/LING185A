...
predecessors (r:rs) s = 
  case r of
  Step x y -> if (y == s) then x : (predecessors rs s) else predecessors rs s
  End x -> predecessors rs s

rulesFromSentence :: [String] -> [GrammarRule]
rulesFromSentence [] = []
rulesFromSentence (x:xs) =
  case xs of
  [] -> End x : []
  (y:ys) -> Step x y : (rulesFromSentence xs)
  

rulesFromText :: [[String]] -> [GrammarRule]
rulesFromText [] = []
rulesFromText l = concat (map rulesFromSentence l)
...
extendByOne g sd =
  case sd of
  Last s -> (map (\s->NonLast s sd) (predecessors grammar1 s))
  NonLast s' sd' -> (map (\s->NonLast s sd) (predecessors grammar1 s'))
