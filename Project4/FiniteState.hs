...
tSB g (w:ws) = 
  case (w:ws) of 
  (w:[])->predecessors g w (concat (enders g))
  (w:ws)->predecessors g w (tSB g ws)
...
takeStepsBack g [w] = (concat (map (\x -> predecessors g w x) (enders g)))
takeStepsBack g (w:ws) =
  concat (map (\endState -> predecessors g w endState) (takeStepsBack g ws))

recognizeBackward :: [GrammarRule] -> State -> [String] -> Bool
recognizeBackward g s [] = False
recognizeBackward g s ws = 
  if elem s (takeStepsBack g ws) then True 
  else False

recognizeForward :: [GrammarRule] -> State -> [String] -> Bool
recognizeForward g s [] = False
recognizeForward g s ws =
  if elem True (map (\z -> elem z (enders g)) (takeStepsForward g s ws)) then True
  else False
...
parse g [] = concat(map (\x -> [Last x]) (enders g))
parse g ws =
  concat (map (\x -> parseHelp g x ws) (takeStepsBack g ws))


parseHelp g s [] = if elem s (enders g) then [Last s] else []
parseHelp g s (w:ws) = 
    map (\sd -> NonLast s w sd) (concat (map (\s' -> parseHelp g s' ws) (successors g s w)))


------------------------------------------
-- Part 3: Generation
predecessorsG :: [GrammarRule] -> State -> [(State, String)]
predecessorsG [] s = []
predecessorsG (r:rest) s =
    case r of
    End _ -> predecessorsG rest s
    Step p w n -> if n == s then ((p, w) : predecessorsG rest s) else predecessorsG rest s

extendByOne :: [GrammarRule] -> StrucDesc -> [StrucDesc]
extendByOne g sd = map (\(t,s) -> NonLast t s sd) (predecessorsG g (firstState sd))
